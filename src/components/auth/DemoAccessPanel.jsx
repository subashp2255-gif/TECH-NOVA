...
export default function DemoAccessPanel({ onAutofill }) {
  ...
  // Hide in production if flag explicitly disabled
  const isDemoEnabled = import.meta.env.VITE_ENABLE_DEMO_LOGIN !== 'false';
    ...
    <div className="border-t border-slate-200/80 pt-3">
      ...
      >
        <span className="flex items-center gap-1.5 uppercase tracking-wider text-[10px]">
          <Sparkles size={12} className="text-amber-500" /> Hackathon Demo Quick Select
        </span>
      ...
      {expanded && (
        <div className="mt-2.5 grid grid-cols-3 gap-2 animate-in fade-in slide-in-from-top-1 duration-150">
          <button
            type="button"
            onClick={() => onAutofill('24AD042', 'student123')}
            className="p-2.5 rounded-xl bg-blue-50/80 border border-blue-200 hover:bg-blue-100 hover:border-blue-300 transition-all text-center group cursor-pointer focus:ring-2 focus:ring-blue-400"
          >
            ...
            <span className="text-[9px] font-mono font-bold text-blue-700 block mt-0.5">24AD042</span>
          </button>

          <button
            type="button"
            onClick={() => onAutofill('LIB001', 'staff123')}
            className="p-2.5 rounded-xl bg-teal-50/80 border border-teal-200 hover:bg-teal-100 hover:border-teal-300 transition-all text-center group cursor-pointer focus:ring-2 focus:ring-teal-400"
          >
            ...
            <span className="text-[9px] font-mono font-bold text-teal-700 block mt-0.5">LIB001</span>
          </button>

          <button
            type="button"
            onClick={() => onAutofill('ADM001', 'admin123')}
            className="p-2.5 rounded-xl bg-indigo-50/80 border border-indigo-200 hover:bg-indigo-100 hover:border-indigo-300 transition-all text-center group cursor-pointer focus:ring-2 focus:ring-indigo-400"
          >
